    ...
    def run(self):
        ...
        try:
            ...
            if self.data_type == 'file':
                ...
                cuckoo_results_url = "http://10.10.100.107:8080/analysis/{}/summary".format(task_id)

            # url analysis

            # url analysis or hash
            elif self.data_type == 'url' or self.data_type == 'hash':
                if self.data_type == 'url':
                    data = {"url": self.get_data()}
                elif self.data_type == 'hash':
                    data = {"hash": self.get_data()}
                else:
            ...
            if self.data_type == 'url' or self.data_type == 'hash':
                self.report({
                    ...
                    'yara': resp_json['target']['url'] if 'target' in resp_json.keys() and 'url' in resp_json[
                        'target'].keys() else '-',
                    'cuckoo_results_url': cuckoo_results_url
                })
            else:
                self.report({
                    'signatures': list_description,
